...
export function parseSingleReplayFromBuffer(buf: Buffer): SingleReplay | null {
    const replayData = safeDecompress(buf);
    const all = findAllJsonInBinary(replayData);
    ...
    for (const obj of all) {
        if (isRecord(obj)) {
            if (hasKeys(obj, ["playerName", "mapDisplayName"])) {
                battle_metadata = obj as BattleMetadata;
            }
            if (hasKeys(obj, ["personal", "common"])) {
                battle_results = obj as BattleResults;
                // не break — іноді metadata може бути після
            }
        }
    }
    ...
    const main_player_name = battle_metadata.playerName;
    if (!main_player_name) return null;

    // знайдемо танк гравця у metadata
    const vehicles_meta = (battle_metadata.vehicles ?? {}) as Record<string, VehicleMeta>;
    let main_player_tank: string | null = null;
    for (const v of Object.values(vehicles_meta)) {
        if ((v?.name ?? "") === main_player_name) {
            main_player_tank = String(v?.vehicleType ?? "N/A").split(":").pop() ?? "N/A";
            break;
        }
    }
    if (!main_player_tank) return null;

    // витягнемо персональні стати
    let player_stats: PersonalStats | null = null;
    const personal = (battle_results.personal ?? {}) as Record<string, PersonalStats>;
    for (const val of Object.values(personal)) {
        if (isRecord(val) && "damageDealt" in (val as PersonalStats)) {
            player_stats = val as PersonalStats;
            break;
        }
    }
...
// ПОВНІСТЮ БЕЗПЕЧНА агрегація: нічого не кидає, збирає помилки
export type ParseSkip = { file: string; reason: string };

export function processReplayFilesWithReport(
    files: { name: string; data: Buffer }[]
): { tank_stats: Record<string, TankAgg>; skipped: ParseSkip[]; processed: number; total: number } {
    const tank_stats: Record<string, TankAgg> = {};
    ...
    for (const f of files) {
        try {
            const data = parseSingleReplayFromBuffer(f.data);
            if (!data) {
                skipped.push({ file: f.name, reason: "Unsupported or non-random replay" });
                continue;
            }
            ...
            skipped.push({ file: f.name, reason: e instanceof Error ? e.message : "Unknown parse error" });
            continue;
        }
    }

    return { tank_stats, skipped, processed, total: files.length };
}
